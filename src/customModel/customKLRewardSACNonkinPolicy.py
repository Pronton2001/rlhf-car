...
def custom_postprocess_trajectory(policy: Policy,
    ...
    # output_logits = inverseUnicycle(pred_x, pred_y, pred_yaw, obs['old_speed']) # B, 1 # [x, y, yaw, old_speed] -> [steer, accel]
    ...
    output_logits = standard_normalizer_nonKin(output_logits) # scale actions # [low, high] -> [-1,1]
    # logging.debug(f'normalize actions: {output_logits}')
    # print('-----------------------------> after normalize', output_logits)

    ones = torch.ones_like(pred_x) 

    std_x, std_y, std_yaw=  -1, -1, -1
    output_logits = torch.cat((output_logits, ones * std_x, ones * std_y, ones * std_yaw), dim = -1)
    # output_logits_std = torch.cat((ones*lx, ones * ly, ones * lyaw), dim = -1)
    
    pretrained_action_dist = TorchDiagGaussian(output_logits, None)
    # pretrained_action_dist2 = TorchSquashedGaussian(output_logits, None)
    # assert pretrained_logits.shape[1] == 6, 'Not (B, 6)'
    ...
    # kl_div2 = kl_approx(sac_action_dist, pretrained_action_dist2)

    # 
    # logging.debug(f'{counter}')
    # if counter == 0:
    #     logging.debug(f'OKKKKKKKKKKKKKK')
    #     kl_weight_decay = policy.model.action_model.kl_div_weight
    #     decay = kl_weight_decay/float(3e5)
    #     counter += len(sample_batch) * 8
    # else:
    #     kl_weight_decay = kl_weight_decay * 1/(1 +  decay* counter)
    #     counter += len(sample_batch)

    


    kl_div = kl_div.cpu().numpy()
    # kl_div2 = kl_div2.cpu().numpy()
    logging.debug(f'KL: {kl_div}')
    # logging.debug(f'REWARDS: { sample_batch[SampleBatch.REWARDS]}')
    sample_batch[SampleBatch.REWARDS] -=  kl_div *  policy.model.action_model.kl_div_weight
    # sample_batch[SampleBatch.REWARDS] -=  kl_div 
    # logging.debug(f'Beta : { kl_div* policy.model.action_model.kl_div_weight}')
    # logging.debug(f'Beta * KL: { kl_div* policy.model.action_model.kl_div_weight}')
    # logging.debug(f'REWARDS - Beta * KL: { sample_batch[SampleBatch.REWARDS]}')
    return postprocess_trajectory(policy, sample_batch, other_agent_batches, episode = None,)

# Build a child class of `TorchPolicy`, given the custom functions defined
# above.

KLRewardSACNonkinTorchPolicy = build_policy_class(
