...
# from .global_graph import MultiheadAttentionGlobalHead, VectorizedEmbedding, MultiheadAttentionGlobalHeadFeatureExtractor
# from .local_graph import LocalSubGraph, SinusoidalPositionalEmbedding

class MultiheadAttentionGlobalHeadFeatureExtractor(nn.Module):
...
class CustomVectorizedModel(nn.Module):
    ...
    def __init__(
        ...
        self.type_embedding = VectorizedEmbedding(self._d_global) # (1 + N(4) + M(8)) x 256 = 13 x 256        self.disable_pos_encode = False
        ...
        # )
        self.global_head = MultiheadAttentionGlobalHeadFeatureExtractor(
                self._d_global, dropout=self._global_head_dropout
            )
    ...
    def model_call(
        ...
        all_avail = torch.cat([agents_avail.to(torch.bool), static_avail.to(torch.bool)], dim=1)
        ...
        feature, attns = self.global_head(all_embs, type_embedding, invalid_polys)
        return feature, attns
    ...
    def forward(self, data_batch: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        feature_outputs, attns = self.model_call(
            agents_polys, map_polys, agents_availabilities, map_availabilities, type_embedding, lane_bdry_len
        )
        return feature_outputs, attns


class CustomLocalGraphModel(nn.Module):
    """ Vectorized planning model.
    """
